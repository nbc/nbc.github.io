---
title: "Comparaison entre arrow::to_arrow() et duckplyr pour écrire du parquet"
...
Une syntaxe souvent recommandée pour écrire un fichier parquet après des ordres `dplyr::tbl` avec `duckdb` est d'utiliser `arrow::to_arrow` avec `arrow::write_dataset` ou `arrow::write_parquet` :
...
Si cette syntaxe fonctionne et sans matérialiser les données, le nouveau package [duckplyr](https://duckplyr.tidyverse.org/index.html) propose une méthode beaucoup plus efficace :
...
1. [`duckplyr::as_duckdb_tibble`](https://duckplyr.tidyverse.org/reference/duckdb_tibble.html) converti l'objet retourné par `tbl` en objet lisible par `duckplyr`
2. [`duckplyr::compute_parquet`](https://duckplyr.tidyverse.org/reference/compute_parquet.html) écrit le fichier parquet

Les deux lignes fonctionnent de la même façon que celles de `arrow`, sans matérialiser les données, mais la conversion d'un objet duckdb en objet duckplyr est **beaucoup plus efficace**.
...
Sur le serveur que j'utilise, la version `duckplyr` est **9 fois plus rapide** que la version `arrow` et consomme **deux fois moins de mémoire**, à égalité avec la méthode pure duckdb (pour ce cas de test très simple).
